...
public class Main {
    public static void main(String[] args) {
        ...
        BiFunction<Double, Double, Double> biF1 = (x1, x2) -> 100 * Math.pow(x2 - x1 * x1, 2) + 5 * Math.pow(1 - x1, 2);
        BiFunction<Double, Double, Double> biF2 = (x1, x2) -> Math.pow(Math.pow(x1, 2) + x2 - 11, 2) + Math.pow(x1 + Math.pow(x2, 2) - 7, 2);
        BiFunction<Double, Double, Double> g = (x, y) -> (x * x + x * y + y * y - 6 * x - 9 * y);

        GradientDescent gd1 = new GradientDescent(biF1, new Point(1, 2), 1e-5);
        double[] result1 = gd1.minimize();
        System.out.println("Градиентный спуск для функции Розенброка: " + result1[0] + "   " + result1[1]);

        GradientDescent gd2 = new GradientDescent(biF2, new Point(1, 10), 1e-5);
...
//      Начальные точки деформируемого треугольника
        ...
        NelderMead nm1 = new NelderMead(biF1, p1, p2, p3, 1000);
        ...
        NelderMead nm2 = new NelderMead(biF2, p1, p2, p3, 1000);
        ...
        ConjugateGradient cg1 = new ConjugateGradient(biF1, new Point(0, 1), 1e-5);
        double[] result5 = cg1.minimize();
        System.out.println("Метод сопряженных градиентов для функции Розенброка: " + result5[0] + "   " + result5[1]);

        ConjugateGradient cg2 = new ConjugateGradient(biF2, new Point(4, -2), 1e-5);
